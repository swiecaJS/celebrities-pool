...
describe('Time for turn ends', () => {
  ...
  it('asks users to enter characters', () => {
    ...
    const provideCharacterName = () => {
      let i = -1;

      return () => {
        console.log('i to', i);
        i += 1;
        if (i === 0 || i === 1) return 'Leo Messi';

        return faker.name.findName();
      };
    };

    const characterProvider = provideCharacterName();
    ...
    cy.wrap(new Array(totalCharactersNumber)).each(() => {
      ...
      cy.get('[data-cy="enter-character"]').type(characterProvider());
  ...
  it('Doubled character is removed', () => {
    const totalCharactersNumber = charactersPerPerson * numberOfPlayers;

    cy.get('[data-cy="characters-left-to-guess"]')
      .invoke('val')
      .then(value => expect(Number(value)).to.eq(totalCharactersNumber - 1));
  });
});
